...
void main()
{
	...
	INPUT_RECORD ir;
	DWORD x, consoleMode;
	SetConsoleCursorInfo(hOut, &cInfo);//устанавливает настройки курсора
	COORD c = { 0, 0 };
	COORD current = { 0, 0 };

	GetConsoleMode(hIn, &consoleMode);//запоминает текущий режим консоли в consoleMode

	while (1)
	{
		SetConsoleMode(hIn, ENABLE_WINDOW_INPUT | ENABLE_MOUSE_INPUT);//устанавливает режим консоли
		while (ReadConsoleInput(hIn, &ir, 1, &x))//считываем буфер действий консоли и помещает его в ir
		{
			if (ir.EventType == MOUSE_EVENT)
			{
				if (ir.Event.MouseEvent.dwButtonState == FROM_LEFT_1ST_BUTTON_PRESSED)
				{
					...
					for (int i = 0; i <= size * 2; ++i)//пробежка по вертикали
					{
						for (int j = 0; j <= size * 2; ++j)//пробежка по горизонтали
						{
							...
							{
								//cout << '*';
								SetConsoleTextAttribute(hOut, FOREGROUND_GREEN | FOREGROUND_INTENSITY | BACKGROUND_BLUE | BACKGROUND_GREEN);//меняет цвет текста
								cout << char(1);
								SetConsoleTextAttribute(hOut, FOREGROUND_BLUE | FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_INTENSITY | BACKGROUND_GREEN | BACKGROUND_BLUE);//меняет цвет текста
							}
							else
							if (i == 0 && j == 0)
								cout << char(0xDA);
				...
				{
					GetConsoleScreenBufferInfo(hOut, &csbInfo);//получает информацию о  консоли
					FillConsoleOutputAttribute(hOut, FOREGROUND_BLUE, csbInfo.dwSize.X*csbInfo.dwSize.Y, c, &x);
					FillConsoleOutputCharacter(hOut, '*', csbInfo.dwSize.X*csbInfo.dwSize.Y, c, &x);
				}
			}
